...
#pragma omp for schedule(dynamic)
	  ...
	  itk::RGBPixel<unsigned char> clProstateColor;
	...
	bool mi2hed::SaveColorProbabilityMap() {
	    typedef itk::RGBPixel<unsigned char> RGBPixelType;
	    typedef itk::Image<RGBPixelType, 3> RGBImageType;
	    typedef itk::Function::JetColormapFunction<short, RGBPixelType> ColormapType;

	    //Get probability map volume
	    std::cout << "Info: Generating volumes" << std::endl;     
            std::vector<std::string> vPatientIds = GetPatientIDs();
            vPatientIds[0]="765252653.MR112564024";
            
            for (int i = 0; i < vPatientIds.size(); ++i){

               std::string strPatientId = GetOutputFolder() + "/" + vPatientIds[i] + ".%d.png";

               //Load reference volumes
               LoadVolumesForPatient(vPatientIds[i]);
               const ImageType::SizeType &clSize = m_p_clT2WIVolume->GetBufferedRegion().GetSize();
               const ImageType::SpacingType &clSpacing = m_p_clT2WIVolume->GetSpacing();
...
#pragma omp for schedule(dynamic)
	  ...
	  p_clRGBProbabilityMap->SetRegions(p_clVolume->GetBufferedRegion());
	  p_clRGBProbabilityMap->SetSpacing(p_clVolume->GetSpacing());
	  ...
	  for (int z = 0; z < clSize[2]; ++z) {
	   ...
	   itksys::SystemTools::MakeDirectory(m_strProbMapOutputFolder.c_str());
	   std::string strOutputFile = m_strProbMapOutputFolder + "/" + vPatientIds[i] + ".mhd";

	   std::cout << "Info: Saving color probability map '" << strOutputFile << "' ..." << std::endl;

	   SaveVolume<RGBPixelType>(p_clRGBProbabilityMap, strOutputFile.c_str());
	}
        return true;
    }
